import java.util.Scanner;

public class Blackjack {

    public static Scanner scan = new Scanner(System.in);

    public static void main(String[] args) {
        System.out.println("\nWelcome to Java Casino!");
        System.out.println("Do you have a knack for Black Jack?");
        System.out.println("We shall see..");
        System.out.println("..Ready? Press anything to begin!");
        scan.nextLine();

        int randomCard1 = drawRandomCard();
        int randomCard2 = drawRandomCard();

        System.out.println("\nYou get a \n" + cardSrting(randomCard1) + "\nand a \n" + cardSrting(randomCard2));

        int handValue = (Math.min(randomCard1, 10) + Math.min(randomCard2,10));
        System.out.println("Your total is: " + handValue);

        int dealerCard1 = drawRandomCard();
        int dealerCard2 = drawRandomCard();
        int dealerHand = Math.min(dealerCard1, 10) + Math.min(dealerCard2, 10);

        System.out.println("The dealer shows \n" + cardSrting(dealerCard1) + "\nand has a card facing down \n" + faceDown());
        System.out.println("\nThe dealer's total is hidden");


        while (true) {
            String option = hitOrStay();

            if (option.equals("stay")) {
                break;
            }

            int newCard = drawRandomCard();
            handValue = handValue + (Math.min(newCard, 10));
            System.out.println("\nYou get a\n" + cardSrting(newCard));
            System.out.println("Your total is " + handValue);

            if (handValue > 21) {
                System.out.println("Bust! Player loses!");
                System.exit(0);
            }

        }
        
        System.out.println("\nDealer's turn");
        System.out.println("\nThe dealer's cards are \n" + cardSrting(dealerCard1) + "\n and a \n" + cardSrting(dealerCard2));
        System.out.println("Dealer's total is " + dealerHand);

        while (dealerHand < 17) {
            int newCard = drawRandomCard();

            dealerHand += Math.min(newCard, 10);
            System.out.println("\n Dealer gets a \n " + cardSrting(newCard));
            System.out.println("Dealer's total is " + dealerHand);
            
        }

        if (dealerHand > 21) {
            System.out.println("Dealer loses");
            System.exit(0);
        }

        if (handValue > dealerHand) {
            System.out.println("Player wins!");
        } 

        System.out.println("Dealer Wins!");

        
        // while (!option.equals("stay")) {
        //     int draw = drawRandomCard();
        //     System.out.println("\nYou got a \n" + cardSrting(draw));
        //     handValue = handValue + (Math.min(draw, 10));
        //     System.out.println("Your new total is: " + handValue);

        //     if (handValue > 21) {
        //         System.out.println("Bust! Player loses!");
        //         System.exit(0);
        //     }
        //     option = hitOrStay();
        // }

        

        // while (handValue <= 21 && dealerHand <= 21) {
        //     System.out.println("Dealer's turn");
        //     System.out.println("The dealer's cards are \n" + cardSrting(dealerCard1) + "\nand a \n" + cardSrting(dealerCard2));
        //     int draw = drawRandomCard();
        //     dealerHand = dealerHand +draw;
        //     System.out.println("Dealer gets a \n" + cardSrting(draw) + "\nDealer's total is " + dealerHand);
        //     if (dealerHand > 21) {
        //         System.out.println("Bust! Dealer loses!");
        //         System.exit(0);
        //     }
        //     option = hitOrStay();
        // }
        
  
        
        scan.close();
    }

    public static int drawRandomCard() {
        double random = Math.random()*13;
        return (int)random+1;
    }


    public static String cardSrting(int cardNumber) {

        switch (cardNumber) {
            case 1: return  "   _____\n"+
                            "  |A _  |\n"+ 
                            "  | ( ) |\n"+
                            "  |(_'_)|\n"+
                            "  |  |  |\n"+
                            "  |____V|\n";

            case 2: return  "   _____\n"+              
                            "  |2    |\n"+ 
                            "  |  o  |\n"+
                            "  |     |\n"+
                            "  |  o  |\n"+
                            "  |____Z|\n";
            case 3: return  "   _____\n" +
                            "  |3    |\n"+
                            "  | o o |\n"+
                            "  |     |\n"+
                            "  |  o  |\n"+
                            "  |____E|\n";
            case 4: return  "   _____\n" +
                            "  |4    |\n"+
                            "  | o o |\n"+
                            "  |     |\n"+
                            "  | o o |\n"+
                            "  |____h|\n";
            case 5: return  "   _____ \n" +
                            "  |5    |\n" +
                            "  | o o |\n" +
                            "  |  o  |\n" +
                            "  | o o |\n" +
                            "  |____S|\n";
            case 6: return  "   _____ \n" +
                            "  |6    |\n" +
                            "  | o o |\n" +
                            "  | o o |\n" +
                            "  | o o |\n" +
                            "  |____6|\n";
            case 7: return  "   _____ \n" +
                            "  |7    |\n" +
                            "  | o o |\n" +
                            "  |o o o|\n" +
                            "  | o o |\n" +
                            "  |____7|\n";
            case 8: return  "   _____ \n" +
                            "  |8    |\n" +
                            "  |o o o|\n" +
                            "  | o o |\n" +
                            "  |o o o|\n" +
                            "  |____8|\n";
            case 9: return  "   _____ \n" +
                            "  |9    |\n" +
                            "  |o o o|\n" +
                            "  |o o o|\n" +
                            "  |o o o|\n" +
                            "  |____9|\n";
            case 10: return "   _____ \n" +
                            "  |10  o|\n" +
                            "  |o o o|\n" +
                            "  |o o o|\n" +
                            "  |o o o|\n" +
                            "  |___10|\n";
            case 11: return "   _____\n" +
                            "  |J  ww|\n"+ 
                            "  | o {)|\n"+ 
                            "  |o o% |\n"+ 
                            "  | | % |\n"+ 
                            "  |__%%[|\n";
            case 12: return "   _____\n" +
                            "  |Q  ww|\n"+ 
                            "  | o {(|\n"+ 
                            "  |o o%%|\n"+ 
                            "  | |%%%|\n"+ 
                            "  |_%%%O|\n";
            case 13: return "   _____\n" +
                            "  |K  WW|\n"+ 
                            "  | o {)|\n"+ 
                            "  |o o%%|\n"+ 
                            "  | |%%%|\n"+ 
                            "  |_%%%>|\n";

            default: return "";
        }
    }

    public static String faceDown() {
        return
        "   _____\n"+
        "  |     |\n"+ 
        "  |  J  |\n"+
        "  | JJJ |\n"+
        "  |  J  |\n"+
        "  |_____|\n";
    }
    

     public static String hitOrStay() {
  
        System.out.println("Hit or Stay?");
        String response = scan.nextLine();

        while (!(response.equalsIgnoreCase("hit") || response.equalsIgnoreCase("stay"))) {
            System.out.println("Please write hit or stay");
            response = scan.nextLine();
        }
        return response;
     }
    }
